...
from engine.platform_win.startup import is_run_at_startup_synced, set_run_at_startup
...
def test_set_run_at_startup_enable():
...
def test_set_run_at_startup_disable():
    ...
    mock_winreg.DeleteValue.assert_called_once_with(
        mock_winreg.OpenKey.return_value, "Voice2Text"
    )

def test_is_run_at_startup_synced_true():
    with patch("sys.executable", r"C:\path\to\voice2text.exe"):
        mock_winreg.QueryValueEx.return_value = (r"C:\path\to\voice2text.exe", mock_winreg.REG_SZ)
