...
class Win32InputMonitor:
    ...
    def set_hotkey(self, hotkey_str: str, hold_mode: bool = False):
        ...
        logger.debug(f"Hotkey Parsed: {hotkey_str} -> Modifiers: {self._modifier_vks}, Main: {self._hotkey_vk}")
    ...
    def _ll_keyboard_handler(self, nCode, wParam, lParam):
        """Native Keyboard Hook Procedure."""
        if nCode < 0:
            return user32.CallNextHookExW(self._hook, nCode, wParam, lParam)
        ...
        base_vk = vk
        if vk in (VK_LCONTROL, VK_RCONTROL): base_vk = VK_CONTROL
        elif vk in (VK_LSHIFT, VK_RSHIFT):   base_vk = VK_SHIFT
        elif vk in (VK_LMENU, VK_RMENU):     base_vk = VK_MENU

        if base_vk in modifier_keys:
            if is_down: self._pressed_modifiers.add(base_vk)
            elif is_up: self._pressed_modifiers.discard(base_vk)
    ...
    def _hook_loop(self):
        ...
        if not self._hook:
            logger.error("Failed to install native WH_KEYBOARD_LL hook.")
            return
    ...
    def start(self):
        ...
        self._thread = threading.Thread(target=self._hook_loop, name="Win32HookThread", daemon=False)
        self._thread.start()
