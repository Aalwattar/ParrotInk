...
## 1. Overview
This track adds a "Run at Startup" feature to ParrotInk, allowing the application to automatically launch when the user logs into Windows. The configuration file remains the source of truth, while a **Registry Value** under the Windows `Run` key is used to implement the actual startup behavior.
...
- Implement logic to manage the Windows Registry **Value** named "ParrotInk" under the key: `HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Run`.
...
- [ ] Enabling the feature creates the "ParrotInk" **value** in the Windows Startup registry.
- [ ] Disabling the feature removes the "ParrotInk" **value** from the registry.
