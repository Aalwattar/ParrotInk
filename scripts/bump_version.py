...
def bump_version(new_version: str):
    ...
    if vinfo_path.exists():
        ...
        v_content = re.sub(r'filevers=\(\d+, \d+, \d+, \d+\)', f'filevers=({major}, {minor}, {patch}, 0)', v_content)
        v_content = re.sub(r'prodvers=\(\d+, \d+, \d+, \d+\)', f'prodvers=({major}, {minor}, {patch}, 0)', v_content)
...
Successfully bumped to {new_version}")
    print("Run 'git add pyproject.toml packaging/pyinstaller/version_info.txt uv.lock' to stage changes.")
