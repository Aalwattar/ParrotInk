...
- Configurable options will include:
    ...
    - **Seamless Background Updates:** The application features a Windows-native update manager that leverages the Background Intelligent Transfer Service (BITS) to download new versions silently in the background. It verifies updates via SHA256 checksums and provides interactive installation prompts. The update process utilizes a deterministic handoff mechanism that scrubs inherited runtime environments, ensuring the new version launches reliably without library conflicts.
