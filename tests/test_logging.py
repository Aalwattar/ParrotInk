...
def test_sanitizing_formatter_redacts_secrets():
    ...
    record = logging.LogRecord(
        "test", logging.INFO, "path", 1, '{"Authorization": "Bearer sk-abc123secret"}', None, None
    )
    ...
    record = logging.LogRecord(
        "test", logging.INFO, "path", 1, '{"api_key": "my-secret-key"}', None, None
    )
    assert "<REDACTED>" in formatter.format(record)
...
def test_sanitizing_formatter_truncates_audio():
    ...
    long_audio = "A" * 500
    record = logging.LogRecord(
        "test", logging.INFO, "path", 1, f'{{"type": "audio", "audio": "{long_audio}"}}', None, None
    )
    ...
    # AssemblyAI style
    record = logging.LogRecord(
        "test", logging.INFO, "path", 1, f'{{"audio_data": "{long_audio}"}}', None, None
    )
    formatted = formatter.format(record)
    assert "<AUDIO_DATA_TRUNCATED>" in formatted
    assert long_audio not in formatted
