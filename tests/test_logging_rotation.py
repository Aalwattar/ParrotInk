...
def test_logging_rotation_creation(temp_log_dir):
    ...
    mock_config.logging.file_level = 0  # DEBUG
    ...
    # 1. Write first batch
    logger.debug("Initial log message that is definitely more than 100 bytes including the format and timestamp and other info.")
    ...
    for i in range(5):
        logger.debug(f"Rotation trigger message {i} - padding to ensure we exceed 100 bytes per message easily.")
        time.sleep(0.05)
