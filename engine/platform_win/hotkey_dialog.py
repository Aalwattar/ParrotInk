...
class HotkeyRecordingWindow:
    """
    A stable HUD-styled modal window that captures a hotkey combination.
    Uses native Win32 window messages (WM_KEYDOWN) to avoid global hooks.
    """
    ...
    def _draw(self):
        """Perform UI drawing with guaranteed resource cleanup."""
        if not self._hwnd:
            return
        ...
        try:
            ...
            gdiplus.GdipSetStringFormatAlign(str_format, 1)
    ...
    def _handle_key_up(self, vk: int):
        # We finalize when ALL keys are released, OR when at least one valid non-modifier
        # combo was pressed.
        if self.current_keys:
            ...
            if self.is_valid_cb(keys_list):
                ...
                # Still invalid, remove the key that was released
    ...
    def show(self):
        ...
        # IMPORTANT: Trigger the callback ONLY AFTER the message loop ends and window is gone
