...
## Overview
Implement a manual build process to produce a standalone, single-file Windows executable (`Voice2Text.exe`) for the application. This includes fixing runtime path resolution for packaged execution and creating a reproducible PyInstaller configuration.

## Functional Requirements
- **Path Management:** Implement `engine/platform_win/paths.py` to resolve application data paths (config, logs) to `%APPDATA%\Voice2Text`.
...
- **Build Configuration:** Create `build/pyinstaller/voice2text_onefile.spec` with necessary hidden imports and binary collections:
...
## Acceptance Criteria
- Running `scripts/build_onefile.ps1` produces `dist/Voice2Text.exe`.
...
- Configuration is saved to and loaded from `%APPDATA%\Voice2Text\config.toml`.
- Logs are written to `%APPDATA%\Voice2Text\voice2text.log`.
